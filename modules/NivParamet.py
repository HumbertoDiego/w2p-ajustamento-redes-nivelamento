...
def parametrico(filename, x , auto, dp_na_4col):
    ...
    for i in range(0, size(c, 0)): # converter numeros com virg para numeros com ponto
        re.append(c[i][0].replace(',','.')) # 1ª col são estações a ré
        vante.append(c[i][1].replace(',','.')) # 2ª col são estações a vante
        observado.append(c[i][2].replace(',','.'))  # 3ª col são os desniveis observados
        print observado[i]
        if c[i][0] == 0:
            dpinjuc.append(c[i][3].replace(',','.'))
            dist.append(0)
            auto = False
            aviso = 'Aviso: Desvio Padrão convertido para manual devido a presença de Injuções'
        else:
            dpinjuc.append(0)
            dist.append(c[i][3].replace(',','.'))
...
def grafico_teste_hipotesebicaudal(quicalc, signif, df):
    ...
    path2 =  os.path.join(os.getcwd(),'applications\se6alunos\static\images\\fdpqui.png')
    print path2
    plt.savefig(path2)
    return dict()
